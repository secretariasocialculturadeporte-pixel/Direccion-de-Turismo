...
class LieutenantTask(BaseModel):
    ...
    responsible_lieutenant: str # This will always be 'Atractivos' for this captain
...
async def create_lieutenant_plan(state: AtractivosCaptainState) -> AtractivosCaptainState:
    print("--- 🧠 CAP. ATRACTIVOS: Creando Plan de Pelotón (SIMULADO)... ---")
    plan = LieutenantPlan(plan=[
        LieutenantTask(task_description=state['coronel_order'], responsible_lieutenant='Atractivos')
    ])
    state.update({"lieutenant_plan": plan, "task_queue": plan.plan.copy(), "completed_missions": []})
    return state

def route_to_lieutenant(state: AtractivosCaptainState):
    if state.get("error") or not state["task_queue"]:
        return "compile_report"
    return "delegate_to_generic_lieutenant"

async def delegate_node(state: AtractivosCaptainState) -> AtractivosCaptainState:
    """
    Invoca al Teniente Genérico, configurándolo para que comande al Sargento de Atractivos.
    """
    mission = state["task_queue"].pop(0)
    lieutenant_name = mission.responsible_lieutenant
    sargento_module_name = "gestion_atractivos_sargento"

    print(f"--- 🚀 CAPITÁN ATRACTIVOS: Delegando a TTE. GENÉRICO para comandar a Sgto. de {lieutenant_name} -> '{mission.task_description}' ---")
...
async def compile_final_report(state: AtractivosCaptainState) -> AtractivosCaptainState:
    if state.get("error"):
        ...
        state["final_report"] = f"Misión de gestión de Atractivos completada. Resumen:\n{report_body}"
    return state
