...
class PrestadorServicioPublicListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        Permite excluir una categoría con el parámetro 'exclude_categoria'.
        """
        queryset = PrestadorServicio.objects.filter(aprobado=True)
        categoria_slug = self.request.query_params.get('categoria', None)
        exclude_categoria_slug = self.request.query_params.get('exclude_categoria', None)

        if categoria_slug:
            queryset = queryset.filter(categoria__slug=categoria_slug)

        if exclude_categoria_slug:
            queryset = queryset.exclude(categoria__slug=exclude_categoria_slug)
