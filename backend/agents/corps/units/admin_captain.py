...
async def create_lieutenant_plan(state: AdminCaptainState) -> AdminCaptainState:
    print("--- 🧠 CAP. ADMIN: Creando Plan de Pelotón (SIMULADO)... ---")
    plan = LieutenantPlan(plan=[
        LieutenantTask(task_description=state['coronel_order'], responsible_lieutenant='Admin')
    ])
...
async def delegate_node(state: AdminCaptainState) -> AdminCaptainState:
    """
    Invoca al Teniente Genérico, configurándolo para que comande al Sargento de Administración.
    """
    mission = state["task_queue"].pop(0)
    lieutenant_name = mission.responsible_lieutenant
    sargento_module_name = "gestion_admin_sargento"

    print(f"--- 🚀 CAPITÁN ADMIN: Delegando a TTE. GENÉRICO para comandar a Sgto. de {lieutenant_name} -> '{mission.task_description}' ---")
...
async def compile_final_report(state: AdminCaptainState) -> AdminCaptainState:
    if state.get("error"):
        ...
        state["final_report"] = f"Misión de Administración completada. Resumen:\n{report_body}"
    return state
